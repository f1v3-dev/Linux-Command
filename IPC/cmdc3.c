...
// *******************************************************************************
// SIGHLD 시그널 처리 함수
// 자식 cmdc3가 종료했을 때 부모 cmdc3에게 CIGCHLD 시그널이 도착
...
static void sig_child(int sig)
{
	...
	exit(0); 	// 자식 cmdc3가 종료하면 시그널을 받아 부모 cmd3가 여기서 종료
}
void dual_process(void)
{
	...
	if ((pid = fork()) < 0)
		...
		wait(NULL);				// 부모가 먼저 끝난 경우, 자식이 종료될 때까지 기다림
	}
	else {						// 자식 프로세스 : 서버가 보내준 메세지를 받아 화면에 출력
		//signal(SIGINT, SIG_IGN);
		close(out_fd);			// 사용하지 않기 때문에 닫음
		recv_output_loop();		// 반복하여 서버에서 받은 데이터를 모니터(화면)로 출력
		printf("child: exit\n");
	}
}
// 문제점: 
