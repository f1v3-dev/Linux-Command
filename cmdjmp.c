...
get_argv_optv(char *cmd_line)
{
	...
	for ( ; (tok = strtok(NULL, " \t\n")) != NULL; ) { // 다음 토큰 잘라냄
		...
		if (tok[0] == '-')			//토큰이 옵션임을 의미. 예) optv[0]->"-s"
			...
			argv[argc++] = tok;		//argv[argc]에 토큰 주소 저자하고 argc++
...
// "ls -l" 옵션을 준 경우 : 디렉토리의 모든 파일에 대해 상세정보 출력하기
static void
...
print_name(DIR *dp)
{
	...
	// max_name_len : 가장 긴 파일이름 길이 + 4(이름 뒤 여유 공간)
...
// argv[0] -> "디렉토리이름"; [디렉토리이름]을 준 경우
// argc = [디렉토리 이름]을 준 경우 1, 주지 않았을 경우 0
void
cd(void)
{
	
	if(argc == 0){	//명령어 인자를 주지 않았을 경우
		struct passwd *pwp;
		pwp = getpwuid(getuid());
		argv[0] = pwp->pw_dir;
		chdir(argv[0]);
...
// 프로그램을 원하는 시간 도앙ㄴ(초 단위) 일시적으로 중지하는 명령어
// 사용법: sleep 초단위시간
void
Sleep(void)
{
...
run_cmd(void)
{
	...
	// fork()의 리턴 값(pid)이 0이면 자식 프로세스이고,
	// 0보다 크면 부모 프로세스임

	// 자식 프로세스는 외부 프로그램으로 자기 자신을 대체한다.
	// 이 과정에서 에러가 발생했으면 에러 원인 출력하고 스스로 종료
	else if (pid == 0) {
		int i, cnt = 0;
		char *nargv[100];
		...
		if (execvp(cmd, nargv) < 0) {	// 외부 프로그램으로 대체
			// 해당 프로그램이나 접근권한이 없거나 또는 실행 프로그램이 아닌 경우 에러
			perror(cmd);		// 여기서 PRINT_ERR_RET()를 호출하면 안됨
			exit(1);			// 에러 발생 시 자식 프로세스는 종료해야 함
		}
	} // 자식 프로세스는 외부 프로그램으로 대체되었기 때문에 이 이후의 코드는 실행되지 않음.


	// fork()의 리턴 값(pid)이 양수 (pid = 자식 프로세스의 ID)이면
	// 부모 프로세스
	// 부모 프로세스 (cmd 프로그램)는 자식 프로세스(외부 프로그램)가
	// 종료될 때까지 더 이상 실해오디지 않고 여기서 대기해야 함.
	// 이 과정에서 에러가 발생했으면 에러 원인 출력하고 스스로 종료한다.
	...
	// 즉, 죽어서 무덤까지 만들어 주었지만 (시스템 내에 종료된 프로그램 제거),
	// 부모가 동 사무소에 사망신고를 하지 않는 경우임
	// (종료된 프로그램의 정보는 남아 있음)
...
main(int argc, char *argv[])
{
	...
	if((jmpret = setjmp(jump)) != 0){		// longjmp()에 의해 리턴된 경우
		if (jmpret == -1)				// PRINT_ERR_RET()에서 longjmp()한 경우
			perror(cmd);					
		else if (jmpret == -2)			// check_arg()와 check_opt()에서 longjmp()한 경우	
		
			print_usage("사용법: ", cmd_tbl[cmd_idx].cmd,
				cmd_tbl[cmd_idx].opt, cmd_tbl[cmd_idx].arg);
			// proc_cmd()에 있는 이 함수호출을 복사하여 삽입하되
			// k 대신에 전역변수 cmd_idx 사용
	}
